        ...
        <nav className="hidden lg:flex items-center gap-7 text-xs font-semibold tracking-[0.15em]">
          {links.map((l, i) => {
            ...
            return (
              ...
              >
                {l.label.toUpperCase()}
                {isActive && <span className="absolute -bottom-1 left-0 right-0 h-[2px] bg-gold" />}
              </Link>
            );
          })}
        </nav>
...
"
      ...
      {open && (
        <div className="lg:hidden border-t border-cream/10 bg-navy-deep px-4 py-4 space-y-3">
          {links.map((l, i) => (
            <Link key={i} to={l.to} onClick={() => setOpen(false)} className="block text-sm font-semibold tracking-wider text-cream hover:text-gold">
              {l.label.toUpperCase()}
            </Link>
          ))}
